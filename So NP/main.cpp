...
void dfs(int s)
{
    vis[s] = 1;          // Mark the current source vertex as visited
    //cout<<s<<" ";           // Print the vertex
    for(int i=0; i<gr[s].size(); i++) // Traverse through all the connected components of vertex s
    {
        if(!vis[gr[s][i]])           // If they are not visited, call dfs for the new connected vertex
            dfs(gr[s][i]);
    }
}
