...
int main()
{
    ...
    ll N;
    cin >> N;
    ll dist[N][N]; for(int i=0;i<N;++i) for(int j=0;j<N;++j) cin >> dist[i][j];
    ...
    while(K--){
    	...
    	for(int i=0;i<N;++i) for(int j=0;j<N;++j) dist[i][j] = dist[j][i] = min(dist[i][j],dist[i][a]+dist[a][j]);
    	for(int i=0;i<N;++i) for(int j=0;j<N;++j) dist[i][j] = dist[j][i] = min(dist[i][j],dist[i][b]+dist[b][j]);
